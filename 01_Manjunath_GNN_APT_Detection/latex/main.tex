...
neighbour-sampling and message-passing.}
...
discriminative information beyond tabular features. Varying layers from 2 to 3
reveals performance degradation (F1: 1.000$\to$0.997), consistent with
over-smoothing \cite{kipf2017,zhou2020}. The neighbour sampling configuration
($S_1=15, S_2=10$) follows GraphSAGE recommendations \cite{hamilton2017};
preliminary experiments with reduced sampling ($S_1=5, S_2=5$) yielded
equivalent performance, further confirming the dominance of node features on
this synthetic benchmark. On more challenging real-world datasets with
overlapping class distributions, graph structure is hypothesised to provide
measurable advantage as lateral movement patterns create topological signatures
invisible to feature-only classifiers.
