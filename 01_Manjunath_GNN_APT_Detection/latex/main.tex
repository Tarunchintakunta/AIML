...
XGBoost, and MLP---on the KDD Cup~99 benchmark dataset (~494K network flow
records, 41 features), demonstrating F1-scores of 1.000 and 0.997 respectively. Statistical analysis
...
Sampling, Network Intrusion Detection, KDD Cup 99, Message Passing, Inductive Learning
\end{IEEEkeywords}
...
The publicly available KDD Cup~99 dataset (auto-downloaded via scikit-learn) is
employed, comprising approximately 494,000 labelled network flow records with
41 features (38 numeric and 3 categorical), with binary labels distinguishing
normal (benign) from attack traffic. CIC-IDS2017 is additionally supported as
an alternative benchmark. The key contributions of this work are:
...
network-flow graphs constructed from KDD Cup~99 benchmark data, with CIC-IDS2017 support.
...
generalisation with neighbour sampling on KDD Cup~99 and CIC-IDS2017, with
...
\textbf{This Work} (2025) & \textbf{GraphSAGE + neighbour sampling on flow graphs} & \textbf{KDD Cup~99, CIC-IDS2017} & \textbf{F1: 1.000} & \textbf{Benchmark data; ceiling effect; offline evaluation} \\
\bottomrule
\end{tabular}
\end{table*}
...
The primary dataset employed is the KDD Cup~99 dataset, a widely-used
benchmark for network intrusion detection research. It is auto-downloaded
via scikit-learn's \texttt{fetch\_kddcup99} utility, ensuring full
reproducibility without manual data acquisition. The dataset comprises
approximately 494,000 network flow records with 41 features: 38 numeric
features capturing connection-level statistics (e.g., duration, byte counts,
error rates) and 3 categorical features (\texttt{protocol\_type},
\texttt{service}, \texttt{flag}). Labels are binarised into two classes:
\textit{normal} (benign) and \textit{attack}, enabling binary
classification for APT-relevant threat detection.

CIC-IDS2017~\cite{sharafaldin2018} is additionally supported as an
alternative benchmark, containing over 2.8 million network flows with 80
...
KDD Cup~99 is one of the most extensively studied benchmarks in network
intrusion detection, originally derived from the 1998 DARPA Intrusion
Detection Evaluation and used in over 2{,}000 published studies. While the
dataset exhibits known limitations---including redundant records and a
skewed class distribution---it remains the de-facto standard for
reproducible IDS benchmarking. Its use enables direct comparison with
the extensive body of prior GNN and traditional ML work on network-flow
classification.

The dataset provides four concrete advantages for this study:
(a)~\textbf{reproducibility}---auto-download via scikit-learn ensures any
researcher can exactly replicate reported results without manual data
acquisition; (b)~\textbf{known ground truth labels}---attack labels are
assigned by controlled testbed execution rather than manual annotation;
(c)~\textbf{rich feature set}---the 41 features capture connection-level,
content-level, and traffic-level statistics suitable for graph-based
analysis; and (d)~\textbf{scale}---approximately 494K records provide
sufficient volume to evaluate graph construction and neighbour sampling
at non-trivial scale.

The class balance after binarisation yields approximately 20\% normal and
80\% attack records. Feature distributions for key discriminative features
(duration, src\_bytes, dst\_bytes, count) are consistent with published
benchmark statistics from Tavallaee et al.~\cite{tavallaee2009}.
...
\begin{enumerate}
    ...
    \item One-hot encoding of the three categorical features
    (\texttt{protocol\_type}, \texttt{service}, \texttt{flag}), expanding
    the feature space accordingly.
    \item Binary label encoding: normal$\,=\,0$, attack$\,=\,1$.
...
campaigns. For the KDD Cup~99 dataset, source and destination host identifiers
are derived from the connection records. Neighbour connections are limited to
...
statistically significant and practically meaningful even on this benchmark
dataset.
...
F1\,=\,1.000 on the KDD Cup~99 network-flow graph, which is comparable to the
...
well-separated class boundaries in the KDD Cup~99 benchmark; on real APT
...
perfect scores on every metric. This is a direct consequence of the
well-separated benign and malicious flow distributions in the KDD Cup~99
benchmark, which produce a largely linearly separable classification boundary.
While these results validate pipeline correctness and CPU tractability, they
do not provide evidence of superiority over simpler methods on more
challenging, noisy data.
...
benchmark dataset, IP-address co-occurrence topology does not provide
...
confirming that node features dominate on this benchmark dataset.
...
benchmark dataset's clean class separation limits dropout's regularisation
benefit.
...
near-perfect separability of the KDD Cup~99 benchmark dataset. When all models achieve
...
\textbf{Benchmark vs.\ Real Data.} KDD Cup~99 and CIC-IDS2017 are
benchmarks derived from controlled network testbeds. Real enterprise traffic
...
\textbf{Label Quality.} KDD Cup~99 labels were assigned by controlled testbed
execution from the 1998 DARPA evaluation, providing known ground truth but
with potential limitations from the controlled environment. CIC-IDS2017
labels were generated by matching generated traffic to known attack time
windows, introducing potential label noise at temporal boundaries.
...
F1\,=\,1.000 on the KDD Cup~99 benchmark). A formal problem definition, complete
...
The primary limitation is the well-separated class boundaries of the KDD Cup~99
benchmark, which produce a ceiling effect masking potential advantages of
graph-structural features on more challenging detection scenarios. Future work
should evaluate the framework on adversarially collected network data with real APT campaign
...
The KDD Cup~99 dataset is auto-downloaded via scikit-learn's
\texttt{fetch\_kddcup99} function. CIC-IDS2017 is publicly available from
the Canadian Institute for Cybersecurity \cite{sharafaldin2018}.
